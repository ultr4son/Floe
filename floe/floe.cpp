...
mpc_ast_t* parse(const char* input)
{
	...
	mpca_lang(MPCA_LANG_DEFAULT,
		...
		" store      : <node> \"<-\" <expression>';';"
		" floe       : /^/ <store>+ /$/;",

		Node, IntLiteral, Expr, Prod, Value, Store, Floe, NULL);

	mpc_result_t r;

	if (mpc_parse("input", input, Floe, &r)) {

		mpc_cleanup(7, Node, IntLiteral, Expr, Prod, Value, Store, Floe);
...
int main()
{
	mpc_ast_t* ast = parse("a <- 2;b <- a + 2 - 4 * 5;");
	mpc_ast_print(ast);
